package com.bitcli.render;

import com.bitcli.render.inline.InlineRenderer;
import com.bitcli.render.inline.TerminalCapabilities;
 ...
 * <p>选型规则：
 * <ul>
 *   <li>{@code -Dbitcli.renderer} > {@code BITCLI_RENDERER} 环境变量 > 默认 inline</li>
 ...
 *   <li>兼容：{@code BITCLI_TUI=true} → 等价 {@code lanterna}，打 deprecation 提示</li>
 * </ul>
 *
...
public final class RendererFactory {
    ...
    public static Mode resolveMode() {
        String prop = System.getProperty("bitcli.renderer");
        if (prop != null && !prop.isBlank()) {
            return parse(prop);
        }
        String env = System.getenv("BITCLI_RENDERER");
        if (env != null && !env.isBlank()) {
            return parse(env);
        }
        // 兼容旧 BITCLI_TUI=true → lanterna
        String legacyTui = System.getenv("BITCLI_TUI");
        if (legacyTui != null && Boolean.parseBoolean(legacyTui.trim())) {
            System.err.println("⚠️ BITCLI_TUI=true 已废弃，请改用 BITCLI_RENDERER=lanterna");
    ...
    private static Mode parse(String raw) {
        return switch (raw.trim().toLowerCase()) {
            ...
            default -> {
                System.err.println("⚠️ 未识别的 BITCLI_RENDERER='" + raw + "'，回退到 inline");
                yield Mode.INLINE;
            }
        };
    }
