...
public class FileTreePane extends Panel {
     ...
     * @param config 配置
     */
    public FileTreePane(BitCliConfig config) {
        super();
     ...
     * 加载忽略规则（用户级 + 内置默认）。
     */
    private static List<String> loadIgnorePatterns(BitCliConfig config) {
        List<String> patterns = new ArrayList<>(DEFAULT_IGNORE);

        // 读取用户级 filetree-ignore.txt
        Path userIgnore = Path.of(System.getProperty("user.home"), ".bitcli", "filetree-ignore.txt");
