...
import com.bitcli.tui.config.TuiConfigPanel;
 ...
 *
 * <p>TUI 触发条件：
 * - {@code BITCLI_RENDERER=lanterna|tui} 环境变量或 {@code -Dbitcli.renderer=lanterna|tui}
 * - 兼容旧入口：{@code BITCLI_TUI=true} 环境变量或 {@code -Dbitcli.tui=true} 系统属性
 *
 * <p>即使显式启用，以下情况仍降级：
...
public final class TuiBootstrap {
    ...
    private static final String TUI_ENV = "BITCLI_TUI";
    private static final String TUI_PROPERTY = "bitcli.tui";
    private static final String RENDERER_ENV = "BITCLI_RENDERER";
    private static final String RENDERER_PROPERTY = "bitcli.renderer";
    ...
    public static boolean shouldUseTui(Terminal terminal) {
        ...
        if (Boolean.parseBoolean(Objects.requireNonNullElse(
            ...
            System.out.println(AnsiStyle.heading("💡 提示: NO_TUI=true，已切换为 CLI 模式。"
                    + "要启用 TUI 请清除 NO_TUI 环境变量，并使用 BITCLI_RENDERER=lanterna。"));
            return false;
        }
     ...
     */
    public static void launch(BitCliConfig config,
                              LlmClient llmClient,
                              Agent reactAgent,
