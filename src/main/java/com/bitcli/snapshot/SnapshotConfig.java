...
public record SnapshotConfig(
    ...
    public static SnapshotConfig fromEnvironment() {
        boolean enabled = readBoolean("bitcli.snapshot.enabled", "BITCLI_SNAPSHOT_ENABLED", true);
        Path root = Path.of(readString("bitcli.snapshot.dir", "BITCLI_SNAPSHOT_DIR",
                Path.of(System.getProperty("user.home"), ".bitcli", "snapshots").toString()));
        int max = readInt("bitcli.snapshot.max", "BITCLI_SNAPSHOT_MAX", 50);
        List<String> excludes = mergeExcludes(readString("bitcli.snapshot.excludes", "BITCLI_SNAPSHOT_EXCLUDES", ""));
