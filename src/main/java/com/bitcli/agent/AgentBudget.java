 ...
 * 1. 系统属性：{@code bitcli.react.token.budget} / {@code bitcli.react.stagnation.window} /
 *    {@code bitcli.react.hard.max.iterations}
 ...
 * 场景（CI / 自动化批跑）通过 {@code -Dbitcli.react.token.budget=N} 显式启用。
...
public class AgentBudget {
    ...
    public static AgentBudget fromLlmClient(LlmClient llmClient) {
        ...
        // 显式 -Dbitcli.react.token.budget=N 仍可启用硬预算，覆盖默认。
        return new AgentBudget(
                readIntProperty("bitcli.react.token.budget", Integer.MAX_VALUE),
                readIntProperty("bitcli.react.stagnation.window", DEFAULT_STAGNATION_WINDOW),
                readIntProperty("bitcli.react.hard.max.iterations", DEFAULT_HARD_MAX_ITERATIONS)
        );
    }
