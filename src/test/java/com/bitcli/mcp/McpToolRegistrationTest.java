...
class McpToolRegistrationTest {
    ...
    private static void withAuditDir(Path tempDir, ThrowingRunnable body) throws Exception {
        String previous = System.getProperty("bitcli.audit.dir");
        System.setProperty("bitcli.audit.dir", tempDir.resolve("audit").toString());
        try {
            body.run();
        } finally {
            if (previous == null) {
                System.clearProperty("bitcli.audit.dir");
            } else {
                System.setProperty("bitcli.audit.dir", previous);
            }
        }
    }
