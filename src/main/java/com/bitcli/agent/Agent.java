package com.bitcli.agent;

import com.bitcli.llm.LlmClient;
import com.bitcli.llm.LlmTraceLogger;
import com.bitcli.context.ContextProfile;
import com.bitcli.context.TokenUsageFormatter;
import com.bitcli.lsp.LspDiagnosticReport;
import com.bitcli.memory.ConversationHistoryCompactor;
import com.bitcli.memory.ExplicitMemoryHints;
import com.bitcli.memory.MemoryManager;
import com.bitcli.prompt.PromptAssembler;
import com.bitcli.prompt.PromptContext;
import com.bitcli.prompt.PromptMode;
import com.bitcli.prompt.ProjectMemoryLoader;
import com.bitcli.render.PlainRenderer;
import com.bitcli.render.Renderer;
import com.bitcli.render.StatusInfo;
import com.bitcli.runtime.CancellationContext;
import com.bitcli.skill.SkillContextBuffer;
import com.bitcli.skill.SkillIndexFormatter;
import com.bitcli.skill.SkillRegistry;
import com.bitcli.util.AnsiStyle;
import com.bitcli.tool.ToolRegistry;
import com.bitcli.tool.ToolRegistry.ToolExecutionResult;
import com.bitcli.tool.ToolRegistry.ToolInvocation;
import com.bitcli.util.TerminalMarkdownRenderer;
import com.bitcli.image.ImageReferenceParser;
...
public class Agent {
    ...
    public String getContextStatus() {
        com.bitcli.context.ContextProfile profile = memoryManager.getContextProfile();
        ...
        for (LlmClient.Message msg : conversationHistory) {
            int t = com.bitcli.memory.TokenBudget.estimateMessagesTokens(java.util.List.of(msg));
            switch (msg.role()) {
    ...
    private int estimateToolsSchemaTokens() {
        try {
            return com.bitcli.memory.MemoryEntry.estimateTokens(
    ...
    private long estimateCurrentContextTokens() {
        long messageTokens = com.bitcli.memory.TokenBudget.estimateMessagesTokens(conversationHistory);
    ...
    private void logRequestContext(String scope, List<LlmClient.Tool> tools) {
        ...
        for (int messageIndex = 0; messageIndex < conversationHistory.size(); messageIndex++) {
            ...
            messages++;
            int tokens = com.bitcli.memory.TokenBudget.estimateMessagesTokens(List.of(msg));
        ...
        if (tools != null && !tools.isEmpty()) {
            try {
                toolsSchemaTokens = com.bitcli.memory.MemoryEntry.estimateTokens(
